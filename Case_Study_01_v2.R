...
all_trips_v2 <- all_trips_v2 %>%
  mutate(change_statio = ifelse(start_station_id == end_station_id, "Mudou", "Manteve")  )
...
all_trips_v2 %>%  ## Tentandp agrupas por mudança de estação
  mutate(weekday = wday(started_at, label = TRUE)) %>%
  group_by(member_casual, weekday, change_statio) %>%  #groups by usertype and weekday
  summarise(number_of_rides = n()							#calculates the number of rides and average duration 
            ,average_duration = mean(ride_length)) %>% 		# calculates the average duration
  arrange(member_casual, weekday, change_statio)
